...
否则，我们需要执行以下三种操作之一：
插入一个字符到 word1 中，变成 word1 的前 i 个字符与 word2 的前 j-1 个字符相同，即 f[i][j] = f[i][j-1] + 1。
删除一个字符，从 word1 中删除字符，使得 word1 的前 i-1 个字符与 word2 的前 j 个字符相同，即 f[i][j] = f[i-1][j] + 1。
...
空间优化：
代码利用一维数组 f 来存储当前行的编辑距离，并且通过 pre 变量保存上一行的状态来减少空间复杂度。
